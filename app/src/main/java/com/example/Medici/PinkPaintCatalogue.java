...
public class PinkPaintCatalogue extends AppCompatActivity {
    ...
    public void paintCollector() throws IOException {
        ...
        while ((thisLine = reader.readLine()) != null) {
            //Searches for instances where PinkIdentifier exists in a row, and adds it to the csvLines while excluding everything else - thus only capturing pink paints
            if (thisLine.contains("PinkIdentifier")){
        ...
        String[][] pinkPaintsArrayCSV = new String[lines.size()][0];
        lines.toArray(pinkPaintsArrayCSV);

        //Initialises XML elements for GUI layout
        TableLayout layout = (TableLayout) findViewById(R.id.cataloguetable);

        //Subjects the application to a control flow loop that runs for the length of the pinkPaintsArrayCSV array
        for(int i=0;i< pinkPaintsArrayCSV.length;i++)
        {
            ...
            //Initialises a variable as the current element of third column of the pinkPaintsArrayCSV array
            int red = Integer.parseInt(pinkPaintsArrayCSV[i][2]);
            //Initialises a variable as the current element of fourth column of the pinkPaintsArrayCSV array
            int green = Integer.parseInt(pinkPaintsArrayCSV[i][3]);
            //Initialises a variable as the current element of fifth column of the pinkPaintsArrayCSV array
            int blue = Integer.parseInt(pinkPaintsArrayCSV[i][4]);
            ...
            textViewName.setText(pinkPaintsArrayCSV[i][1] + " \n" + pinkPaintsArrayCSV[i][5] + " \n" + hexCode);
            ...
            //Converts the current element in the pinkPaintsArrayCSV array into integer, and intialises the redPass variable
            final int redPass = Integer.parseInt(pinkPaintsArrayCSV[i][2]);
            //Converts the current element in the pinkPaintsArrayCSV array into integer, and intialises the greenPass variable
            final int greenPass = Integer.parseInt(pinkPaintsArrayCSV[i][3]);
            //Converts the current element in the pinkPaintsArrayCSV array into integer, and intialises the bluePass variable
            final int bluePass = Integer.parseInt(pinkPaintsArrayCSV[i][4]);
            ...
            paintLinearLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(PinkPaintCatalogue.this, PaintSummary.class);
