...
public class YellowPaintCatalogue extends AppCompatActivity {
    ...
    public void paintCollector() throws IOException {
        ...
        while ((thisLine = reader.readLine()) != null) {
            //Searches for instances where YellowIdentifier exists in a row, and adds it to the csvLines while excluding everything else - thus only capturing yellow paints
            if (thisLine.contains("YellowIdentifier")){
        ...
        String[][] yellowPaintsArrayCSV = new String[lines.size()][0];
        lines.toArray(yellowPaintsArrayCSV);

        //Initialises XML elements for GUI layout
        TableLayout layout = (TableLayout) findViewById(R.id.cataloguetable);

        //Subjects the application to a control flow loop that runs for the length of the yellowPaintsArrayCSV array
        for(int i=0;i< yellowPaintsArrayCSV.length;i++)
        {
            ...
            //Initialises a variable as the current element of third column of the yellowPaintsArrayCSV array
            int red = Integer.parseInt(yellowPaintsArrayCSV[i][2]);
            //Initialises a variable as the current element of fourth column of the yellowPaintsArrayCSV array
            int green = Integer.parseInt(yellowPaintsArrayCSV[i][3]);
            //Initialises a variable as the current element of fifth column of the yellowPaintsArrayCSV array
            int blue = Integer.parseInt(yellowPaintsArrayCSV[i][4]);
            ...
            textViewName.setText(yellowPaintsArrayCSV[i][1] + " \n" + yellowPaintsArrayCSV[i][5] + " \n" + hexCode);
            ...
            //Converts the current element in the yellowPaintsArrayCSV array into integer, and intialises the redPass variable
            final int redPass = Integer.parseInt(yellowPaintsArrayCSV[i][2]);
            //Converts the current element in the yellowPaintsArrayCSV array into integer, and intialises the greenPass variable
            final int greenPass = Integer.parseInt(yellowPaintsArrayCSV[i][3]);
            //Converts the current element in the yellowPaintsArrayCSV array into integer, and intialises the bluePass variable
            final int bluePass = Integer.parseInt(yellowPaintsArrayCSV[i][4]);
            ...
            paintLinearLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(YellowPaintCatalogue.this, PaintSummary.class);
