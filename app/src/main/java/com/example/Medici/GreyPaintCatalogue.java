...
public class GreyPaintCatalogue extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_grey_paint_catalogue);
    ...
    public void paintCollector() throws IOException {
        ...
        List<String[]> csvLnes = new ArrayList<String[]>();
        while ((thisLine = reader.readLine()) != null) {
            //Searches for instances where GreyIdentifier exists in a row, and adds it to the csvLines while excluding everything else - thus only capturing black paints
            if (thisLine.contains("GreyIdentifier")){
                //Uses semi-colon delimiter to recognise columns and rows of data
                csvLnes.add(thisLine.split(";"));
            }
        }

        //Forces a substring to copy to a new underlying line array
        String[][] greyPaintsArrayCSV = new String[csvLnes.size()][0];
        csvLnes.toArray(greyPaintsArrayCSV);

        //Initialises XML elements for GUI layout
        TableLayout layout = (TableLayout) findViewById(R.id.cataloguetable);

        //Subjects the application to a control flow loop that runs for the length of the greyPaintsArrayCSV array
        for(int i=0;i< greyPaintsArrayCSV.length;i++)
        {
            ...
            //Initialises a variable as the current element of third column of the greyPaintsArrayCSV array
            int paintRed = Integer.parseInt(greyPaintsArrayCSV[i][2]);
            //Initialises a variable as the current element of fourth column of the greyPaintsArrayCSV array
            int paintGreen = Integer.parseInt(greyPaintsArrayCSV[i][3]);
            //Initialises a variable as the current element of fifth column of the greyPaintsArrayCSV array
            int paintBlue = Integer.parseInt(greyPaintsArrayCSV[i][4]);
            ...
            paintRed = (paintRed) & 0xff;
            paintGreen = (paintGreen) & 0xff;
            paintBlue = (paintBlue) & 0xff;

            //Calls getHexFromRGB() method and passes the red, green and blue values for encoding into hexadecimal
            hexCode = getHexFromRGB(Color.rgb(paintRed, paintGreen, paintBlue));
            ...
            textViewName.setText(greyPaintsArrayCSV[i][1] + " \n" + greyPaintsArrayCSV[i][5] + " \n" + hexCode);
            ...
            //Converts the current element in the greyPaintsArrayCSV array into integer, and intialises the redPass variable
            final int redPass = Integer.parseInt(greyPaintsArrayCSV[i][2]);
            //Converts the current element in the greyPaintsArrayCSV array into integer, and intialises the greenPass variable
            final int greenPass = Integer.parseInt(greyPaintsArrayCSV[i][3]);
            //Converts the current element in the greyPaintsArrayCSV array into integer, and intialises the bluePass variable
            final int bluePass = Integer.parseInt(greyPaintsArrayCSV[i][4]);
            ...
            paintLinearLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(GreyPaintCatalogue.this, PaintSummary.class);
