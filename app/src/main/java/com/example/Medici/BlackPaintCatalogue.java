...
public class BlackPaintCatalogue extends AppCompatActivity {
    ...
    public void paintCollector() throws IOException {
        ...
        while ((thisLine = reader.readLine()) != null) {
            //Searches for instances where BlackIdentifier exists in a row, and adds it to the csvLines while excluding everything else - thus only capturing black paints
            if (thisLine.contains("BlackIdentifier")){
        ...
        String[][] blackPaintsArrayCSV = new String[csvLnes.size()][0];
        csvLnes.toArray(blackPaintsArrayCSV);

        //Initialises XML elements for GUI layout
        TableLayout layout = (TableLayout) findViewById(R.id.cataloguetable);

        //Subjects the application to a control flow loop that runs for the length of the blackPaintsArrayCSV array
        for(int i=0;i< blackPaintsArrayCSV.length;i++)
        {
            ...
            //Initialises a variable as the current element of third column of the blackPaintsArrayCSV array
            int paintRed = Integer.parseInt(blackPaintsArrayCSV[i][2]);
            //Initialises a variable as the current element of fourth column of the blackPaintsArrayCSV array
            int paintGreen = Integer.parseInt(blackPaintsArrayCSV[i][3]);
            //Initialises a variable as the current element of fifth column of the blackPaintsArrayCSV array
            int paintBlue = Integer.parseInt(blackPaintsArrayCSV[i][4]);
            ...
            textViewName.setText(blackPaintsArrayCSV[i][1] + " \n" + blackPaintsArrayCSV[i][5] + " \n" + hexCode);
            ...
            //Converts the current element in the blackPaintsArrayCSV array into integer, and intialises the redPass variable
            final int redPass = Integer.parseInt(blackPaintsArrayCSV[i][2]);
            //Converts the current element in the blackPaintsArrayCSV array into integer, and intialises the greenPass variable
            final int greenPass = Integer.parseInt(blackPaintsArrayCSV[i][3]);
            //Converts the current element in the blackPaintsArrayCSV array into integer, and intialises the bluePass variable
            final int bluePass = Integer.parseInt(blackPaintsArrayCSV[i][4]);
            ...
            paintLinearLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(BlackPaintCatalogue.this, PaintSummary.class);
