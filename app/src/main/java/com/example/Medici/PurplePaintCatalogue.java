...
public class PurplePaintCatalogue extends AppCompatActivity {
    ...
    public void paintCollector() throws IOException {
        ...
        while ((thisLine = reader.readLine()) != null) {
            //Searches for instances where PurpleIdentifier exists in a row, and adds it to the csvLines while excluding everything else - thus only capturing purple paints
            if (thisLine.contains("PurpleIdentifier")){
        ...
        String[][] purplePaintsArrayCSV = new String[lines.size()][0];
        lines.toArray(purplePaintsArrayCSV);

        //Initialises XML elements for GUI layout
        TableLayout layout = (TableLayout) findViewById(R.id.cataloguetable);

        //Subjects the application to a control flow loop that runs for the length of the purplePaintsArrayCSV array
        for(int i=0;i< purplePaintsArrayCSV.length;i++)
        {
            ...
            //Initialises a variable as the current element of third column of the purplePaintsArrayCSV array
            int red = Integer.parseInt(purplePaintsArrayCSV[i][2]);
            //Initialises a variable as the current element of fourth column of the purplePaintsArrayCSV array
            int green = Integer.parseInt(purplePaintsArrayCSV[i][3]);
            //Initialises a variable as the current element of fifth column of the purplePaintsArrayCSV array
            int blue = Integer.parseInt(purplePaintsArrayCSV[i][4]);
            ...
            textViewName.setText(purplePaintsArrayCSV[i][1] + " \n" + purplePaintsArrayCSV[i][5] + " \n" + hexCode);
            ...
            //Converts the current element in the purplePaintsArrayCSV array into integer, and intialises the redPass variable
            final int redPass = Integer.parseInt(purplePaintsArrayCSV[i][2]);
            //Converts the current element in the purplePaintsArrayCSV array into integer, and intialises the greenPass variable
            final int greenPass = Integer.parseInt(purplePaintsArrayCSV[i][3]);
            //Converts the current element in the purplePaintsArrayCSV array into integer, and intialises the bluePass variable
            final int bluePass = Integer.parseInt(purplePaintsArrayCSV[i][4]);
            ...
            paintLinearLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(PurplePaintCatalogue.this, PaintSummary.class);
