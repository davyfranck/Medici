...
public class BluePaintCatalogue extends AppCompatActivity {
    ...
    public void paintCollector() throws IOException {
        ...
        while ((thisLine = reader.readLine()) != null) {
            //Searches for instances where BlueIdentifier exists in a row, and adds it to the csvLines while excluding everything else - thus only capturing blue paints
            if (thisLine.contains("BlueIdentifier")){
        ...
        String[][] bluePaintsArrayCSV = new String[lines.size()][0];
        lines.toArray(bluePaintsArrayCSV);

        //Initialises XML elements for GUI layout
        TableLayout layout = (TableLayout) findViewById(R.id.cataloguetable);

        //Subjects the application to a control flow loop that runs for the length of the bluePaintsArrayCSV array
        for(int i=0;i< bluePaintsArrayCSV.length;i++)
        {
            ...
            //Initialises a variable as the current element of third column of the bluePaintsArrayCSV array
            int red = Integer.parseInt(bluePaintsArrayCSV[i][2]);
            //Initialises a variable as the current element of fourth column of the bluePaintsArrayCSV array
            int green = Integer.parseInt(bluePaintsArrayCSV[i][3]);
            //Initialises a variable as the current element of fifth column of the bluePaintsArrayCSV array
            int blue = Integer.parseInt(bluePaintsArrayCSV[i][4]);
            ...
            textViewName.setText(bluePaintsArrayCSV[i][1] + " \n" + bluePaintsArrayCSV[i][5] + " \n" + hexCode);
            ...
            //Converts the current element in the bluePaintsArrayCSV array into integer, and intialises the redPass variable
            final int redPass = Integer.parseInt(bluePaintsArrayCSV[i][2]);
            //Converts the current element in the bluePaintsArrayCSV array into integer, and intialises the greenPass variable
            final int greenPass = Integer.parseInt(bluePaintsArrayCSV[i][3]);
            //Converts the current element in the bluePaintsArrayCSV array into integer, and intialises the bluePass variable
            final int bluePass = Integer.parseInt(bluePaintsArrayCSV[i][4]);
            ...
            paintLinearLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(BluePaintCatalogue.this, PaintSummary.class);
