...
public class SilverPaintCatalogue extends AppCompatActivity {
    ...
    public void paintCollector() throws IOException {
        ...
        while ((thisLine = reader.readLine()) != null) {
            //Searches for instances where SilverIdentifier exists in a row, and adds it to the csvLines while excluding everything else - thus only capturing silver paints
            if (thisLine.contains("SilverIdentifier")){
        ...
        String[][] silverPaintsArrayCSV = new String[lines.size()][0];
        lines.toArray(silverPaintsArrayCSV);

        //Initialises XML elements for GUI layout
        TableLayout layout = (TableLayout) findViewById(R.id.cataloguetable);

        //Subjects the application to a control flow loop that runs for the length of the silverPaintsArrayCSV array
        for(int i=0;i< silverPaintsArrayCSV.length;i++)
        {
            ...
            //Initialises a variable as the current element of third column of the silverPaintsArrayCSV array
            int red = Integer.parseInt(silverPaintsArrayCSV[i][2]);
            //Initialises a variable as the current element of fourth column of the silverPaintsArrayCSV array
            int green = Integer.parseInt(silverPaintsArrayCSV[i][3]);
            //Initialises a variable as the current element of fifth column of the silverPaintsArrayCSV array
            int blue = Integer.parseInt(silverPaintsArrayCSV[i][4]);
            ...
            textViewName.setText(silverPaintsArrayCSV[i][1] + " \n" + silverPaintsArrayCSV[i][5] + " \n" + hexCode);
            ...
            //Converts the current element in the silverPaintsArrayCSV array into integer, and intialises the redPass variable
            final int redPass = Integer.parseInt(silverPaintsArrayCSV[i][2]);
            //Converts the current element in the silverPaintsArrayCSV array into integer, and intialises the greenPass variable
            final int greenPass = Integer.parseInt(silverPaintsArrayCSV[i][3]);
            //Converts the current element in the silverPaintsArrayCSV array into integer, and intialises the bluePass variable
            final int bluePass = Integer.parseInt(silverPaintsArrayCSV[i][4]);
            ...
            paintLinearLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(SilverPaintCatalogue.this, PaintSummary.class);
