...
public class OrangePaintCatalogue extends AppCompatActivity {
    ...
    public void paintCollector() throws IOException {
        ...
        while ((thisLine = reader.readLine()) != null) {
            //Searches for instances where OrangeIdentifier exists in a row, and adds it to the csvLines while excluding everything else - thus only capturing orange paints
            if (thisLine.contains("OrangeIdentifier")){
        ...
        String[][] orangePaintsArrayCSV = new String[lines.size()][0];
        lines.toArray(orangePaintsArrayCSV);

        //Initialises XML elements for GUI layout
        TableLayout layout = (TableLayout) findViewById(R.id.cataloguetable);

        //Subjects the application to a control flow loop that runs for the length of the orangePaintsArrayCSV array
        for(int i=0;i< orangePaintsArrayCSV.length;i++)
        {
            ...
            //Initialises a variable as the current element of third column of the orangePaintsArrayCSV array
            int red = Integer.parseInt(orangePaintsArrayCSV[i][2]);
            //Initialises a variable as the current element of fourth column of the orangePaintsArrayCSV array
            int green = Integer.parseInt(orangePaintsArrayCSV[i][3]);
            //Initialises a variable as the current element of fifth column of the orangePaintsArrayCSV array
            int blue = Integer.parseInt(orangePaintsArrayCSV[i][4]);
            ...
            textViewName.setText(orangePaintsArrayCSV[i][1] + " \n" + orangePaintsArrayCSV[i][5] + " \n" + hexCode);
            ...
            //Converts the current element in the orangePaintsArrayCSV array into integer, and intialises the redPass variable
            final int redPass = Integer.parseInt(orangePaintsArrayCSV[i][2]);
            //Converts the current element in the orangePaintsArrayCSV array into integer, and intialises the greenPass variable
            final int greenPass = Integer.parseInt(orangePaintsArrayCSV[i][3]);
            //Converts the current element in the orangePaintsArrayCSV array into integer, and intialises the bluePass variable
            final int bluePass = Integer.parseInt(orangePaintsArrayCSV[i][4]);
            ...
            paintLinearLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(OrangePaintCatalogue.this, PaintSummary.class);
