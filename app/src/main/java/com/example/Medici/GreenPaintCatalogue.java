...
public class GreenPaintCatalogue extends AppCompatActivity {
    ...
    public void paintCollector() throws IOException {
        ...
        while ((thisLine = reader.readLine()) != null) {
            //Searches for instances where BlueIdentifier exists in a row, and adds it to the csvLines while excluding everything else - thus only capturing green paints
            if (thisLine.contains("GreenIdentifier")){
        ...
        String[][] greenPaintsArrayCSV = new String[lines.size()][0];
        lines.toArray(greenPaintsArrayCSV);

        //Initialises XML elements for GUI layout
        TableLayout layout = (TableLayout) findViewById(R.id.cataloguetable);

        //Subjects the application to a control flow loop that runs for the length of the greenPaintsArrayCSV array
        for(int i=0;i< greenPaintsArrayCSV.length;i++)
        {
            ...
            //Initialises a variable as the current element of third column of the greenPaintsArrayCSV array
            int red = Integer.parseInt(greenPaintsArrayCSV[i][2]);
            //Initialises a variable as the current element of fourth column of the greenPaintsArrayCSV array
            int green = Integer.parseInt(greenPaintsArrayCSV[i][3]);
            //Initialises a variable as the current element of fifth column of the greenPaintsArrayCSV array
            int blue = Integer.parseInt(greenPaintsArrayCSV[i][4]);
            ...
            textViewName.setText(greenPaintsArrayCSV[i][1] + " \n" + greenPaintsArrayCSV[i][5] + " \n" + hexCode);
            ...
            //Converts the current element in the greenPaintsArrayCSV array into integer, and intialises the redPass variable
            final int redPass = Integer.parseInt(greenPaintsArrayCSV[i][2]);
            //Converts the current element in the greenPaintsArrayCSV array into integer, and intialises the greenPass variable
            final int greenPass = Integer.parseInt(greenPaintsArrayCSV[i][3]);
            //Converts the current element in the greenPaintsArrayCSV array into integer, and intialises the bluePass variable
            final int bluePass = Integer.parseInt(greenPaintsArrayCSV[i][4]);
            ...
            paintLinearLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(GreenPaintCatalogue.this, PaintSummary.class);
