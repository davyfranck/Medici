...
public class GoldPaintCatalogue extends AppCompatActivity {
    ...
    public void paintCollector() throws IOException {
        ...
        while ((thisLine = reader.readLine()) != null) {
            //Searches for instances where GoldIdentifier exists in a row, and adds it to the csvLines while excluding everything else - thus only capturing gold paints
            if (thisLine.contains("GoldIdentifier")){
        ...
        String[][] goldPaintsArrayCSV = new String[lines.size()][0];
        lines.toArray(goldPaintsArrayCSV);

        //Initialises XML elements for GUI layout
        TableLayout layout = (TableLayout) findViewById(R.id.cataloguetable);

        //Subjects the application to a control flow loop that runs for the length of the goldPaintsArrayCSV array
        for(int i=0;i< goldPaintsArrayCSV.length;i++)
        {
            ...
            //Initialises a variable as the current element of third column of the goldPaintsArrayCSV array
            int red = Integer.parseInt(goldPaintsArrayCSV[i][2]);
            //Initialises a variable as the current element of fourth column of the goldPaintsArrayCSV array
            int green = Integer.parseInt(goldPaintsArrayCSV[i][3]);
            //Initialises a variable as the current element of fifth column of the goldPaintsArrayCSV array
            int blue = Integer.parseInt(goldPaintsArrayCSV[i][4]);
            ...
            textViewName.setText(goldPaintsArrayCSV[i][1] + " \n" + goldPaintsArrayCSV[i][5] + " \n" + hexCode);
            ...
            //Converts the current element in the goldPaintsArrayCSV array into integer, and intialises the redPass variable
            final int redPass = Integer.parseInt(goldPaintsArrayCSV[i][2]);
            //Converts the current element in the goldPaintsArrayCSV array into integer, and intialises the greenPass variable
            final int greenPass = Integer.parseInt(goldPaintsArrayCSV[i][3]);
            //Converts the current element in the goldPaintsArrayCSV array into integer, and intialises the bluePass variable
            final int bluePass = Integer.parseInt(goldPaintsArrayCSV[i][4]);
            ...
            paintLinearLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(GoldPaintCatalogue.this, PaintSummary.class);
