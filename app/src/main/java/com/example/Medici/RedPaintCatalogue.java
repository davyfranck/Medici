...
public class RedPaintCatalogue extends AppCompatActivity {
    ...
    public void paintCollector() throws IOException {
        ...
        while ((thisLine = reader.readLine()) != null) {
            //Searches for instances where RedIdentifier exists in a row, and adds it to the csvLines while excluding everything else - thus only capturing red paints
            if (thisLine.contains("RedIdentifier")){
        ...
        String[][] redPaintsArrayCSV = new String[lines.size()][0];
        lines.toArray(redPaintsArrayCSV);

        //Initialises XML elements for GUI layout
        TableLayout layout = (TableLayout) findViewById(R.id.cataloguetable);

        //Subjects the application to a control flow loop that runs for the length of the redPaintsArrayCSV array
        for(int i=0;i< redPaintsArrayCSV.length;i++)
        {
            ...
            //Initialises a variable as the current element of third column of the redPaintsArrayCSV array
            int red = Integer.parseInt(redPaintsArrayCSV[i][2]);
            //Initialises a variable as the current element of fourth column of the redPaintsArrayCSV array
            int green = Integer.parseInt(redPaintsArrayCSV[i][3]);
            //Initialises a variable as the current element of fifth column of the redPaintsArrayCSV array
            int blue = Integer.parseInt(redPaintsArrayCSV[i][4]);
            ...
            textViewName.setText(redPaintsArrayCSV[i][1] + " \n" + redPaintsArrayCSV[i][5] + " \n" + hexCode);
            ...
            //Converts the current element in the redPaintsArrayCSV array into integer, and intialises the redPass variable
            final int redPass = Integer.parseInt(redPaintsArrayCSV[i][2]);
            //Converts the current element in the redPaintsArrayCSV array into integer, and intialises the greenPass variable
            final int greenPass = Integer.parseInt(redPaintsArrayCSV[i][3]);
            //Converts the current element in the redPaintsArrayCSV array into integer, and intialises the bluePass variable
            final int bluePass = Integer.parseInt(redPaintsArrayCSV[i][4]);
            ...
            paintLinearLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(RedPaintCatalogue.this, PaintSummary.class);
