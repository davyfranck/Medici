...
public class BrownPaintCatalogue extends AppCompatActivity {
    ...
    public void paintCollector() throws IOException {
        ...
        while ((thisLine = reader.readLine()) != null) {
            //Searches for instances where BrownIdentifier exists in a row, and adds it to the csvLines while excluding everything else - thus only capturing brown paints
            if (thisLine.contains("BrownIdentifier")){
        ...
        String[][] brownPaintsArrayCSV = new String[lines.size()][0];
        lines.toArray(brownPaintsArrayCSV);

        //Initialises XML elements for GUI layout
        TableLayout layout = (TableLayout) findViewById(R.id.cataloguetable);

        //Subjects the application to a control flow loop that runs for the length of the brownPaintsArrayCSV array
        for(int i=0;i< brownPaintsArrayCSV.length;i++)
        {
            ...
            //Initialises a variable as the current element of third column of the brownPaintsArrayCSV array
            int red = Integer.parseInt(brownPaintsArrayCSV[i][2]);
            //Initialises a variable as the current element of fourth column of the brownPaintsArrayCSV array
            int green = Integer.parseInt(brownPaintsArrayCSV[i][3]);
            //Initialises a variable as the current element of fifth column of the brownPaintsArrayCSV array
            int blue = Integer.parseInt(brownPaintsArrayCSV[i][4]);
            ...
            textViewName.setText(brownPaintsArrayCSV[i][1] + " \n" + brownPaintsArrayCSV[i][5] + " \n" + hexCode);
            ...
            //Converts the current element in the brownPaintsArrayCSV array into integer, and intialises the redPass variable
            final int redPass = Integer.parseInt(brownPaintsArrayCSV[i][2]);
            //Converts the current element in the brownPaintsArrayCSV array into integer, and intialises the greenPass variable
            final int greenPass = Integer.parseInt(brownPaintsArrayCSV[i][3]);
            //Converts the current element in the brownPaintsArrayCSV array into integer, and intialises the bluePass variable
            final int bluePass = Integer.parseInt(brownPaintsArrayCSV[i][4]);
            ...
            paintLinearLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(BrownPaintCatalogue.this, PaintSummary.class);
